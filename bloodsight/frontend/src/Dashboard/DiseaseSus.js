...
function DiseaseSus({ userId }) {
    const [content, setContent] = useState('Loading...');

    useEffect(() => {
        if (userId) {
            axios.get(`${API_URL}/get_disease_sus/${userId}`)
                .then(response => {
    ...
    return (
        <div>
            <h1 className="text-2xl font-bold mb-4">Disease Susceptibility</h1>
